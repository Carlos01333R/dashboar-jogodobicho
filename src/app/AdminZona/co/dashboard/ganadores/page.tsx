...
import { useWinSummaryAdminZona } from '@/hook/co/adminZona/useInfoWin';
...
const WinnersHistoryAdminZona: React.FC = () => {
  const { selectedCountry,  user} = useAuthAdminZona();
  const zona = user?.sector || ""
  const [dateFilter, setDateFilter] = useState('month');
  const [lotteryFilter, setLotteryFilter] = useState('all');
  const [searchTerm, setSearchTerm] = useState('');
  const { summary, loading, error } = useWinSummaryAdminZona(zona);
...
export default WinnersHistoryAdminZona;