...
export function ModalVentasZonas({ zona }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <button className="w-full bg-emerald-50 hover:bg-emerald-100 text-emerald-700 py-2 px-3 rounded-lg text-sm font-medium transition-colors cursor-pointer flex items-center gap-x-2 justify-center">
          <DollarSign className="w-4 h-4" />
          <span>Ver ventas</span>
        </button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[725px] max-h-[90vh] flex flex-col overflow-y-scroll">
        <DialogHeader>
          <DialogTitle className="flex-1 text-center">Ventas zona {zona}</DialogTitle>
        </DialogHeader>
        {selectedCountry === 'brazil' ? (
        <section>
          {open && (loadingbr || loadingPremio || loadingRewards) && (
          ...
          {(error || errorPremio || errorRewards) && (
            ...
            <section>
          {open && (loading || loadingPremio || loadingRewards) && (
            <div className="flex justify-center items-center py-8">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-emerald-500"></div>
              <p className="ml-4">Cargando datos...</p>
            </div>
